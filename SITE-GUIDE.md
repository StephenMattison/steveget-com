...
#### 5.3.3.2a `llms.txt` for agentic browsing (Mandatory on every site)

Ship a root **`/llms.txt`** so AI agents and future “agentic browsing” checks can understand the site without scraping the full DOM. This is a Markdown plain-text file (not HTML). Spec spirit: [llmstxt.org](https://llmstxt.org/).

**Why:** PageSpeed / agent-accessibility audits already flag missing or invalid `llms.txt` (e.g. “missing H1”, “no links”). Treat this as launch hygiene for AI discovery the same way we treat `sitemap.xml` for traditional crawl.

**Hard requirements (fail agentic checks if missing):**

1. File lives at site root: `llms.txt` → live `https://<canonical-domain>/llms.txt`
2. Served as **real text**, not a soft-404 HTML homepage (confirm `Content-Type: text/plain` and body starts with `# `)
3. **Exactly one H1** at the top: `# Site Name` (Markdown ATX heading — not Setext underlines)
4. **At least one Markdown link** of the form `[Label](https://…)` (bare URLs alone may fail “contains links” checks)
...
8. Keep under ~10 KB; no HTML, JS, or tracking pixels inside the file
...
- Add a comment in `robots.txt` pointing operators to `/llms.txt` (optional; not a robots directive).
- In Cloudflare Pages `_headers`, set explicit `Content-Type: text/plain; charset=utf-8` for `/llms.txt` so edge quirks never return HTML.
- After deploy: `curl -sI https://<domain>/llms.txt` must not return `text/html`, and `curl -sL` must show the `#` H1 and Markdown links.
- Update `llms.txt` when primary nav URLs, phone, pricing, or positioning change.
- Optional later: `llms-full.txt` for long docs; **do not** put secrets, internal runbooks, or unpublished pricing hacks in either file.

**Launch gate:** Agentic / “llms.txt recommendations” check passes (H1 present + links present + file is text). Category may still be experimental (e.g. 2/3 while other agent checks evolve) — still ship a correct file.
